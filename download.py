...
#url = "https://edit.pdc.cms.gov/sites/default/files/archive/Doctors%20and%20clinicians"
url = "https://data.cms.gov/provider-data/sites/default/files/archive/Doctors%20and%20clinicians"
...
for mon_diff in range(0, retrieval_range): 
    ...
    files_to_download.append(f"doctors_and_clinicians_{monthyear}.zip")
...
files_downloaded = [x for x in Path(volumepath_zip).glob("doctors_and_clinicians*")]
...
for file_downloaded in files_downloaded:
    with zipfile.ZipFile(file_downloaded, "r") as zip_ref:
        ...
        for member in zip_ref.namelist():
            if (not Path(f"{volumepath_root}/doctors_and_clinicians/{year}_{month}/{member}").exists()):
                print(f"Extracting {member}...")
                try: 
                    zip_ref.extract(member, path=f"{volumepath_root}/doctors_and_clinicians/{year}_{month}")
